...
import {  useSchedules } from "@/lib/hooks/schedule";
...
const SchedulesTable = () => {
  ...
  const { data, isLoading, isError, error } = useSchedules(allQueries);
  const schedules = data?.data;
  const formattedSchedules = schedules
    ? getSchedulesByDay(schedules)
    : [];
                ...
                {schedules.map((s : ISchedule) => {
                  return (
                    <button
                      ...
                      type="button"
                      className={cn(
                        "flex justify-start items-center gap-1 sm:gap-1.5 rounded-lg border px-1.5 py-1 sm:px-2 md:px-2.5 sm:py-1.5 text-xs sm:text-sm transition border-border bg-surface text-foreground",
                      )}
                    >
