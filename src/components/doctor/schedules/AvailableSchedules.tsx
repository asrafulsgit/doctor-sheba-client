...
import {
  useCreateDoctorSchedules,
  useDoctorAvailableSchedules,
...
const AvailableSchedules = () => {
  ...
  const { mutate: createSchedules, isPending } = useCreateDoctorSchedules();
  ...
  const handleAcceptSchedules = () => {
    ...
    createSchedules(
      { schedules: selectedSchedules },
      {
        ...
        onError: (err: any) => {
          toast.error(err?.message || "Error while creating schedules!");
        },
      },
    );
  };
    ...
    <div>
      ...
      <div className="flex justify-end mt-3">
        <Button
          size="sm"
          variant={"default"}
          onClick={handleAcceptSchedules}
          disabled={!selectedSchedules.length}
        >
          {isPending ? "Creating..." : "Create schedules"}
        </Button>
      </div>
    </div>
  ) : (
