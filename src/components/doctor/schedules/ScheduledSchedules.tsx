...
import {
  ScheduledSchedule,
  useDeleteDoctorSchedules,
  useDoctorScheduledSchedules,
} from "@/lib/hooks/schedule";
...
const ScheduledSchedules = () => {
  ...
  const { data, isLoading, isError, error } =
    useDoctorScheduledSchedules(allQueries);
  const schedules = data?.data;
  const formattedSchedules = schedules ? getSchedulesByDay(schedules) : [];
  const { mutate: deleteSchedules, isPending } = useDeleteDoctorSchedules();
  ...
  const handleDeleteSchedules = () => {
    ...
    deleteSchedules(
      { schedules: selectedSchedules },
      {
        ...
        onError: (err: any) => {
          toast.error(err?.message || "Error while deleting schedules!");
        },
      },
    );
  };
  ...
  return formattedSchedules.length ? (
    <div>
          ...
          return (
            ...
            >
              ...
              <div className="flex flex-col gap-2">
                {schedules.map((s: ScheduledSchedule) => {
                  ...
                  return (
                    <button
                      ...
                      disabled={s.isBooked}
                      className={cn(
                        "flex justify-start items-center gap-1 sm:gap-1.5 rounded-lg border px-1.5 py-1 sm:px-3 sm:py-1.5 text-xs sm:text-sm transition",
                        selected
                          ? "border-border cursor-pointer bg-surface text-foreground hover:border-primary/50 hover:bg-primary-soft"
                          : s.isBooked
                            ? "border-destructive cursor-not-allowed bg-destructive text-destructive-foreground shadow-sm"
                            : "border-primary cursor-pointer bg-primary text-primary-foreground shadow-sm",
                      )}
                    >
                      {selected ? (
      ...
      <div className="flex justify-end mt-3">
        ...
        >
          {isPending ? "Deleting..." : "Delete schedules"}
        </Button>
      </div>
    </div>
  ) : (
